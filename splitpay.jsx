      ...
      <div style={{ background:"#FFF8F0", border:"1px solid #FFE0C8", borderRadius:12,
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontSize:11, color:"#C08060", marginTop:1, lineHeight:1.5 }}>
            {fmtDate(entry.date)} · {nA} {fmt(entry.snapshot?.totA??0)} / {nB} {fmt(entry.snapshot?.totB??0)}
          </div>
        </div>
...
export default function App() {
  const [books,   setBooks]   = useState([{ id:"b1", name:"日常費" }]);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await saveEntries([{id:genId(),bookId,type:"payment",user:sheet,amount:n,memo:memo.trim(),date:new Date().toISOString()},...entries]);
      setSheet(null); setAmount(""); setMemo("");
    } finally { setSubmitting(false); }
  };

  const handleReset = async () => {
    await saveEntries([{id:genId(),bookId,type:"reset",snapshot:{totA,totB},date:new Date().toISOString()},...entries]);
    setShowReset(false);
  };
  ...
  const handleAddBook = async () => {
    ...
    await saveBooks([...books,{id:genId(),name:newBookName.trim()}]);
    setNewBookName("");
  };
    ...
    <div style={{ fontFamily:font,background:BG,minHeight:"100vh",maxWidth:430,margin:"0 auto",
      ...
      <div style={{ background:"#fff",padding:"16px 20px 0",borderBottom:"1px solid #EDE9E2",
        ...
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12 }}>
          <div style={{ fontSize:18,fontWeight:700,color:"#1A1A1A",letterSpacing:"-0.5px",fontFamily:numFont }}>わりかん帳</div>
          ...
          <div style={{ background:"#fff",borderRadius:22,padding:"28px 24px 22px",
            ...
            {diff===0 ? (
              <>
                ...
                <div style={{ fontSize:38,fontWeight:700,color:"#22C55E",fontFamily:numFont,letterSpacing:"-1px" }}>均等！ 🎉</div>
              ...
              <>
                ...
                <div style={{ display:"inline-block",padding:"3px 12px",borderRadius:20,
                  ...
                  {diff>0?names.A:names.B} の方が多く払っています
                </div>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24 }}>
            {["A","B"].map((u)=>(
              <button key={u} onClick={()=>setSheet(u)} style={{ background:uColor(u),color:"#fff",border:"none",
                ...
                <span style={{ fontSize:22,lineHeight:1 }}>＋</span>{names[u]} 払い
              </button>
            ))}
          </div>
        ...
        <>
          ...
          <div style={{ position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",
            ...
            <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:22 }}>
              <div style={{ width:34,height:34,borderRadius:10,background:uColor(sheet),
                display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:14,fontWeight:700,color:"#fff",fontFamily:numFont }}>
                {(names[sheet]||sheet)[0]?.toUpperCase()}
              </div>
              <div style={{ fontSize:17,fontWeight:700,color:"#1A1A1A" }}>{names[sheet]} の支払いを記録</div>
            </div>
              ...
              <div style={{ position:"relative" }}>
                ...
                <input autoFocus inputMode="numeric" value={amount}
                  ...
                  style={{ width:"100%",padding:"14px 14px 14px 38px",fontSize:32,fontWeight:700,
                    border:"2px solid",borderColor:uColor(sheet),borderRadius:14,
            ...
            <button onClick={handleSubmit} disabled={!amount||submitting} style={{ width:"100%",padding:"17px",
              background:!amount?"#E0DDD8":uColor(sheet),color:"#fff",border:"none",borderRadius:16,
              fontSize:16,fontWeight:700,cursor:!amount?"default":"pointer",fontFamily:font,
              boxShadow:!amount?"none":`0 6px 20px ${uColor(sheet)}50` }}>
              {submitting?"記録中…":"記録する ✓"}
            </button>
          </div>
        </>
      ...
      {showReset && (
        <>
          ...
          <div style={{ position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",
            ...
            padding:"8px 24px 32px",zIndex:101,boxShadow:"0 -8px 40px rgba(0,0,0,0.15)",boxSizing:"border-box" }}>
            <div style={{ width:36,height:4,background:"#E0DDD8",borderRadius:2,margin:"12px auto 24px" }}/>
            <div style={{ fontSize:18,fontWeight:700,color:"#1A1A1A",marginBottom:10 }}>🔄 差額をリセット</div>
            ...
            {confirmDelete.type==="reset" ? (
              <div style={{ background:"#FFF8F0",border:"1px solid #FFE0C8",borderRadius:12,
                ...
                <div style={{ fontSize:12,color:"#C08060",marginTop:4 }}>
                  {fmtDate(confirmDelete.date)} · {names.A} {fmt(confirmDelete.snapshot?.totA??0)} / {names.B} {fmt(confirmDelete.snapshot?.totB??0)}
                </div>
              </div>
            ) : (
              <div style={{ background:"#F5F3EE",borderRadius:12,padding:"12px 14px",marginBottom:20 }}>
                ...
                <div style={{ fontSize:12,color:"#999",marginTop:4 }}>
                  {fmtDate(confirmDelete.date)} · {names[confirmDelete.user]||confirmDelete.user}
